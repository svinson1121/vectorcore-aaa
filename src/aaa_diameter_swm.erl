...
-define(CALLBACK_MOD, aaa_diameter_swm_cb).
-define(DIAMETER_DICT_SWm, diameter_3gpp_ts29_273_swm).
...
-define(SWM_VENDOR_ID_3GPP, 10415).
-define(SWM_VENDOR_ID_ETSI, 13019).
-define(DIAMETER_APP_ID_SWm, ?DIAMETER_DICT_SWm:id()).
...
-define(SERVICE,
	[{'Origin-Host', application:get_env(?ENV_APP_NAME, dia_swm_origin_host, ?ENV_DEFAULT_ORIG_HOST)},
	 {'Origin-Realm', application:get_env(?ENV_APP_NAME, dia_swm_origin_realm, ?ENV_DEFAULT_ORIG_REALM)},
	 {'Vendor-Id', application:get_env(?ENV_APP_NAME, dia_swm_vendor_id, ?ENV_DEFAULT_VENDOR_ID)},
	 {'Vendor-Specific-Application-Id',
		[#'diameter_base_Vendor-Specific-Application-Id'{
		 'Vendor-Id'           = ?SWM_VENDOR_ID_3GPP,
		 'Auth-Application-Id' = [?DIAMETER_APP_ID_SWm]}]},
	 {'Product-Name', "VectorCore AAA"},
	 {'Supported-Vendor-Id', [?SWM_VENDOR_ID_3GPP, ?SWM_VENDOR_ID_ETSI]},
	 {application,
	  [{alias, ?APP_ALIAS},
...
init([]) ->
	TxTimer = application:get_env(?ENV_APP_NAME, dia_swm_transmit_timer, ?ENV_DEFAULT_DIAMETER_TRANSMIT_TIMER_MS),
	TableId = ets:new(auth_req, [bag, named_table]),
	{ok, #swm_state{table_id = TableId, tx_timeout = TxTimer}}.
...
handle_cast({der_auth_req, Imsi, PdpTypeNr, Apn, EAP}, State) ->
	...
	aaa_ue_fsm:ev_rx_access_auth_request(Pid, {swm, s2b, undefined, undefined,
...
handle_cast({der_auth_compl_req, Imsi, Apn}, State) ->
	case aaa_ue_fsm:get_pid_by_imsi(Imsi) of
	Pid when is_pid(Pid) ->
		aaa_ue_fsm:ev_rx_access_auth_complete(Pid, {swm, Apn, ok});
	undefined ->
		ok
	end,
	{noreply, State};
...
handle_cast({raa, Imsi, Result}, State) ->
	case aaa_ue_fsm:get_pid_by_imsi(Imsi) of
	Pid when is_pid(Pid) -> aaa_ue_fsm:ev_rx_access_reauth_answer(Pid, {swm, Result});
	undefined -> ok
	end,
	{noreply, State};
...
handle_cast({str, Imsi}, State) ->
	case aaa_ue_fsm:get_pid_by_imsi(Imsi) of
	Pid when is_pid(Pid) ->
		case aaa_ue_fsm:ev_rx_access_session_termination(Pid, swm) of
...
handle_call({rar, Imsi}, _From, State) ->
	lager:debug("SWm Tx RAR IMSI=~p~n", [Imsi]),
	SessionId = diameter:session_id(application:get_env(?ENV_APP_NAME, dia_swm_origin_host, ?ENV_DEFAULT_ORIG_HOST)),
...
handle_call({asr, Imsi}, _From, State) ->
	lager:debug("SWm Tx ASR IMSI=~p~n", [Imsi]),
	SessionId = diameter:session_id(application:get_env(?ENV_APP_NAME, dia_swm_origin_host, ?ENV_DEFAULT_ORIG_HOST)),
...
handle_call({register_pending, Imsi, Kind, Pid}, _From, State) ->
	ets:insert(State#swm_state.table_id, {{Kind, Imsi}, Pid}),
	{reply, ok, State};

handle_call({cancel_pending, Imsi, Kind}, _From, State) ->
	ets:delete(State#swm_state.table_id, {Kind, Imsi}),
	{reply, ok, State};
...
notify_pending(State, Kind, Imsi, Msg, Fallback) ->
	Key = {Kind, Imsi},
	case ets:lookup(State#swm_state.table_id, Key) of
	[{Key, Pid} | _] ->
		ets:delete(State#swm_state.table_id, Key),
		Pid ! Msg,
		ok;
	[] ->
		Fallback()
...
diameter_call(Msg, State) ->
	diameter:call(?SVC_NAME, ?APP_ALIAS, Msg, [{timeout, State#swm_state.tx_timeout},
						   detach]).
